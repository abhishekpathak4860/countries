...
export default function CountryDetails() {
        ...
        setCountryData(
            {
                ...
                tld: data.tld[0],
                currencies: data.currencies ? Object.values(data.currencies)[0].symbol : "no currency",
                languages: data.languages ? Object.values(data.languages).join(", ") : "no language",
                borders: data.borders,
                image: data.flags.svg,

            })
        if (data.borders) {// agar border hai tabhi iske ander aana ok
    ...
    return (
        <>
            {!countryData ? <CountryDetailShimmer /> : countryData && <main className={`${mode ? "dark" : ""}`}>
