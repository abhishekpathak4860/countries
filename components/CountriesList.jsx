...
export default function CountriesList({ query, query1 }) {
  ...
  useEffect(() => {// useEffect is use to make code run only once.
  ...
  const filterCountry = countriesData.filter((countryInfo) => {
    if (query) {
      return countryInfo.name.common.toLowerCase().includes(query.toLowerCase());
    }
    if (query1) {
  ...
  return (
    <>
      {!countriesData.length ? <CountriesListShimmer /> : <div className="countries-container">
        {
          filterCountry.length > 0 ? (filterCountry.map((countryInfo) => (
            <CountryCard
              ...
              capital={countryInfo.capital ? countryInfo.capital : "No Capital"}
              image={countryInfo.flags.svg}
              data={countryInfo}
            />
          ))) :
            (<p>No such country found
            </p>)
        }
      </div>}
    </>
